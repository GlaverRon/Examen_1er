<%@page import="com.emergentes.modelo.Registro"%>
<% 
    Registro reg = (Registro)request.getAttribute("miobjper");
%>

<%@page contentType="text/html" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
        <title>JSP Page</title>
    </head>
    <body>
    <center>
        <div>
        <div>
        <table border="1">
            <td>
                <p>PRIMER PARCIAL TEM-742<p>
                <p>Nombre: Neustan Eliezer Choque Mamani<p>
                <p>Carnet: 9947416 LP.<p>
            </td>
        </table>
        </div>
        <h1>Registro de Calificaciones</h1>
        <form action="MainServlet" method="POST">
            <table>
                <tr>
                    <td><input type="hidden" name="id" value="<%= reg.getId() %>" size="2" readonly></td>
                </tr>
                <tr>
                    <td>Nombre de Estudiante :</td>
                    <td><input type="text" name="nombre" value="<%= reg.getNombre() %>"></td>
                </tr>
                <tr>
                    <td>Primer parcial(sobre 30 pts.) :</td>
                    <td>
                        <input type="text" name="primero" size="3" value="<%= reg.getPrimero() %>">
                    </td>
                </tr>
                <tr>
                    <td>Segundo Parcial(sobre 30 pts.) :</td>
                    <td>
                        <input type="text" name="segundo" size="3" value="<%= reg.getSegundo() %>">
                    </td>
                </tr>
                <tr>
                    <td>Examen Final(sobre 40 pts.) :</td>
                    <td>
                        <input type="text" name="tercero" size="3" value="<%= reg.getTercero() %>">
                    </td>
                </tr>
                <tr>
                    <td></td>
                    <td><input type="submit" value="Enviar"></td>
                </tr>
            </table>

        </form>
       </div>
    </center>
    </body>
</html>
